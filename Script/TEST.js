WidgetMetadata = {
  id: "forward.tmdb.thai.media.collections",
  title: "TMDB 泰國影視合集",
  ...
  modules: [
    {
      ...
      functionName: "tmdbDiscoverThaiTv",
      cacheDuration: 3600,
      params: [
        {
...
async function fetchTmdbData(api, discoverParams, mediaType) {
  try {
    ...
    return results
      ...
      .map((item) => {
        const itemMediaType = item.media_type || mediaType || (item.title ? "movie" : "tv");
        return {
          id: item.id,
          type: "tmdb",
          tmdbId: item.id,
          tmdbTypeId: `${itemMediaType}.${item.id}`,
          ...
          link: `https://www.themoviedb.org/${itemMediaType}/${item.id}`
        };
      });
  } catch (error) {
