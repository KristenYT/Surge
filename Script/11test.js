...
// prettier-ignore
function jxh(e) { const n = e.reduce((e, n) => { const t = e.find((e) => e.name === n.name); if (t) { t.count++; t.items.push({ ...n, name: n.name.endsWith(FNAME) ? `${n.name}` : `${n.name}${XHFGF}${t.count.toString().padStart(2, "0")}`,
 }); } else { e.push({ name: n.name, count: 1, items: [{ ...n, name: n.name.endsWith(FNAME) ? `${n.name}` : `${n.name}${XHFGF}01`,
