...
async function getDirectInfo(ip, provider) {
  ...
  let CN_INFO
  let isCN
  const msg = `使用 ${provider || 'pingan'} 查詢 ${ip ? ip : '分流'} 信息`
  if (provider == 'cip') {
    try {
    ...
    try {
      ...
      isCN = $.lodash_get(body, 'data.location.0') === '中國'
      ...
      CN_INFO = [
        [
          ...
          .filter(i => i)
          .join(' '),
        ['運營商:', $.lodash_get(body, 'data.location.4')].filter(i => i).join(' '),
      ]
        .filter(i => i)
        .join('\n')
    } catch (e) {
    ...
    try {
      ...
      isCN = $.lodash_get(body, 'data.country') === '中國'
    ...
    try {
      ...
      CN_INFO = [
        [
          '位置:',
          getflag(countryCode),
          $.lodash_get(body, 'data.showapi_res_body.country').replace(/\s*中國\s*/, ''),
    ...
    try {
      ...
      CN_INFO = [
        [
          '位置:',
          getflag(countryCode),
          $.lodash_get(body, 'data.country').replace(/\s*中國\s*/, ''),
          $.lodash_get(body, 'data.province'),
          $.lodash_get(body, 'data.city'),
...
async function ali(ip, key) {
  ...
  INFO = [
    [
      '位置:',
      getflag(countryCode),
      $.lodash_get(body, 'result.nation').replace(/中國\s*/, ''),
      $.lodash_get(body, 'result.province'),
      $.lodash_get(body, 'result.city'),
