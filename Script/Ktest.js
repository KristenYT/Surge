...
const ZHT = ['香港', '澳門', '台灣', '日本', '韓國', '新加坡', '美國', '英國', '法國', '德國', '澳大利亞', '阿聯酋', '阿富汗', '阿爾巴尼亞', 
  ...
  '柬埔寨', '喀麥隆', '加拿大', '佛得角', '開曼群島', '中非共和國', '乍得', '智利', '哥倫比亞', '科摩羅', '剛果（布）', 
  '剛果（金）', '哥斯達黎加', '克羅地亞', '賽普勒斯', '捷克', '丹麥', '吉布提', '多明尼加', '厄瓜多', '埃及', '薩爾瓦多', 
  '赤道幾內亞', '厄立特里亞', '愛沙尼亞', '衣索比亞', '斐濟', '芬蘭', '加蓬', '岡比亞', '喬治亞', '加納', '希臘', '格陵蘭', 
  '危地馬拉', '幾內亞', '蓋亞那', '海地', '洪都拉斯', '匈牙利', '冰島', '印度', '印尼', '伊朗', '伊拉克', '愛爾蘭', '馬恩島', 
  '以色列', '義大利', '科特迪瓦', '牙買加', '約旦', '哈薩克', '肯亞', '科威特', '吉爾吉斯', '老撾', '拉脫維亞', '黎巴嫩', 
  '賴索托', '賴比瑞亞', '利比亞', '立陶宛', '盧森堡', '馬其頓', '馬達加斯加', '馬拉威', '馬來西亞', '馬爾代夫', '馬里', '馬耳他', 
  '毛利塔尼亞', '模里西斯', '墨西哥', '摩爾多瓦', '摩納哥', '蒙古', '黑山', '摩洛哥', '莫三比克', '緬甸', '納米比亞', '尼泊爾', 
  '荷蘭', '紐西蘭', '尼加拉瓜', '尼日', '奈及利亞', '北韓', '挪威', '阿曼', '巴基斯坦', '巴拿馬', '巴拉圭', '秘魯', '菲律賓', 
  '葡萄牙', '波多黎各', '卡達', '羅馬尼亞', '俄羅斯', '盧旺達', '聖馬力諾', '沙烏地阿拉伯', '塞內加爾', '塞爾維亞', '塞拉利昂', 
  '斯洛伐克', '斯洛維尼亞', '索馬利亞', '南非', '西班牙', '斯里蘭卡', '蘇丹', '蘇里南', '史瓦濟蘭', '瑞典', '瑞士', '敘利亞', 
  '塔吉克', '坦桑尼亞', '泰國', '多哥', '東加', '千里達', '突尼西亞', '土耳其', '土庫曼', '美屬維爾京群島', '烏干達', '烏克蘭', 
  '烏拉圭', '烏茲別克', '委內瑞拉', '越南', '也門', '尚比亞', '辛巴威', '安道爾', '留尼旺', '波蘭', '關島', '梵蒂岡', '列支敦斯登', 
  '庫拉索', '塞席爾', '南極洲', '直布羅陀', '古巴', '法羅群島', '奧蘭群島', '百慕達', '東帝汶'];
// prettier-ignore
...
const nameclear =
  /(官方|网址|群|客服|网站|获取|下次|官址|联系|邮箱|工单|学术|USE|USED|EMAIL)/i;
// prettier-ignore
...
const rurekey = {
  ...
  台湾: /(深|沪|呼|京|广|杭)美|波特兰|芝加哥|哥伦布|纽约|硅谷|俄勒冈|西雅图|芝加哥/g,
  波斯尼亚和黑塞哥维那: /波黑共和国/g,
  印尼: /印度尼西亚|雅加达/g,
  印度: /孟买/g,
  阿联酋: /迪拜|阿拉伯联合酋长国/g,
  ...
  Object.keys(rurekey).forEach((ikey) => {
    if (rurekey[ikey].test(e.name)) {
      ...
      if (BLKEY) {
        ...
        retainKey = re
          ? BLKEY_REPLACE
          : BLKEY.split("+").filter((items) => e.name.includes(items));
      }
    }
  });
...
// 更新节点名称并生成序号
pro = pro.reduce((acc, item) => {
  const index = acc.findIndex(e => e.name === item.name);
  if (index !== -1) {
    const count = acc[index].count++;
    acc[index].items.push({ 
      ...item, 
      name: `${item.name}${XHFGF}${superscriptMapping[count]}` // 使用上标字符
    });
  } else {
    acc.push({
      name: item.name,
      ...
      items: [{ 
        ...item, 
        name: `${item.name}${XHFGF}${superscriptMapping[0]}` // 初始为上标0
      }]
    });
  }
  return acc;
...
return pro;
  ion getList(arg) { switch (arg) { case 'zht': return ZHT;case 'us': return EN; case 'gq': return FG; case 'quan': return QC; default: return ZH; }}
// prettier-ignore
