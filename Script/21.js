...
function jxh(e) {
  const groups = e.reduce((acc, currentItem) => {
    ...
    if (existingGroup) {
      ...
      existingGroup.items.push({
        ...currentItem,
        name: `${currentItem.name} ${toSuperscript(existingGroup.count.toString().padStart(2, "0"))} ${re ? BLKEY_REPLACE : BLKEYS.filter((items) => e.name.includes(items))} ${FNAME}`
      });
    } else {
      acc.push({
        ...
        items: [{
          ...currentItem,
          name: `${currentItem.name} ${re ? BLKEY_REPLACE : BLKEYS.filter((items) => e.name.includes(items))} ${FNAME}`
        }],
      });
    }
    return acc;
  ...
  groups.forEach(group => {
    if (group.count > 1) {
      // 更新第一个元素的名称以包含序号“01”
      group.items[0].name = `${group.name} ${toSuperscript("01")} ${re ? BLKEY_REPLACE : BLKEYS.filter((items) => e.name.includes(items))}`;
    }
  });
