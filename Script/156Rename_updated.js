 ...
 *** 保留参数
 * [blkey=
bblkey=iplc+gpt+NF+IPLC] 用+号添加多个关键词 保留节点名的自定义字段 需要区分大小写!
 * 如果需要修改 保留的关键词 替换成别的 可以用 > 分割 例如 [#blkey=
bblkey=GPT>新名字+其他关键词] 这将把【GPT】替换成【新名字】
 * 例如      https://raw.githubusercontent.com/Keywos/rule/main/rename.js#flag&blkey=
bblkey=GPT>新名字+NF
 * [blgd]   保留: 家宽 IPLC ˣ² 等
...
function jxh(e) {
  ...
  groups.forEach(group => {
    if (group.count > 1) {
      // 更新第一个元素的名称以包含序号“01”
      group.items[0].name = `${group.name} ${toSuperscript("01")} bblkey=${FNAME}`;
    }
  });
