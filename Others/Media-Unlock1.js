...
async function testDisneyPlus() {
  let { region, cnbl } = await Promise.race([getLocation(), timeout(7000)]);
  if (!region) {
    return { region: null, status: STATUS_TIMEOUT };
  }
  let { status } = await Promise.race([testHomePage(), timeout(7000)]);
  if (status === STATUS_AVAILABLE && cnbl === 1) {
...
function getLocation() {
  return new Promise((resolve, reject) => {
    ...
    $httpClient.get(option, function (error, response, data) {
      ...
      let match = data.match(/Region: ([A-Za-z]{2})[\s\S]*?CNBL: ([01])/);
      if (match && match.length === 3) {
        resolve({ region: match[1], cnbl: parseInt(match[2]) });
        return;
      }

      resolve({ region: null, cnbl: 0 });
    });
  });
}
...
function testHomePage() {
  return new Promise((resolve, reject) => {
    ...
    $httpClient.get(option, function (error, response, data) {
      ...
      if (response.status === 200) {
        resolve({ status: STATUS_AVAILABLE });
        return;
      }
      if (response.status === 403) {
